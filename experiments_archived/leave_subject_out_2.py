...
train = [load_ea_data(vid=v)[0] for v in [0, 1, 2]]
test = [load_ea_data(vid=v)[0] for v in [3]]
m, q, _ = train[0].shape
...
for p in tqdm(p_range, desc='total', ncols=100):
    ...
    # test the model
    err = []
    for test_sub in tqdm(range(200), desc=f'testing: p = {p}', ncols=100):
    # for test_sub in tqdm(range(100), desc=f'p = {p}', ncols=100):
